...
function svgData(label: string, index: number) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="900"><rect width="100%" height="100%" fill="%23e8e4dc"/><text x="50%" y="48%" text-anchor="middle" font-family="Arial" font-size="48" fill="%23343b38">${safe}</text><text x="50%" y="56%" text-anchor="middle" font-family="Arial" font-size="24" fill="%23616b66">Benchmark visual ${index + 1}</text></svg>`;
...
export default async function MultiIndustryCandidatePage({ params }: { params: Promise<{ fixture: string; id: string }> }) {
  ...
  const primaryLabel = entry.fixture.conversionActions[0] ?? "Enquire";
  const primaryAction = { label: primaryLabel, href: "#contact" };
  ...
  const galleryItems = copy.services.map((title, index) => ({ src:svgData(`${entry.fixture.label}: ${title}`, index), alt:`${entry.fixture.label} benchmark visual for ${title}`, caption:title }));
  ...
  const contactProps = { eyebrow:"Contact", headline:`Continue with ${brand}.`, body:"This benchmark form is intentionally non-functional until a real backend is provisioned.", status:"Benchmark enquiry preview", submitLabel:primaryLabel, note:"No submission is sent from this benchmark route." };
