...
export default async function MultiIndustryCandidatePage({ params }: { params: Promise<{ fixture: string; id: string }> }) {
  ...
  const requestedMediaRoles = [heroSupportsMedia ? "hero" : null, sections.services === "services-visual-stories" ? "services" : null, sections.gallery ? "gallery" : null].filter((role): role is "hero" | "services" | "gallery" => Boolean(role));
  ...
  const realMediaCount = [...heroMedia, ...serviceMedia, ...(resolvedMedia.gallery ?? [])].length;
  ...
  return <main style={style} data-benchmark-fixture={fixtureId} data-candidate-id={candidate.id} data-primary-capability={primaryCapability.id} data-primary-capability-status={primaryCapability.status} data-media-source={realMediaCount > 0 ? "provider" : "fallback"} data-media-provider-count={realMediaCount} data-hero-media-source={heroSupportsMedia ? (heroMedia.length ? "provider" : "fallback") : "not-applicable"}>{candidate.sectionOrder.map((type,index)=>{ const sectionType=type as ChoreographedSectionType; return <div key={`${type}-${index}`} style={getSectionColorStyle(candidate.theme,candidate.direction,sectionType) as CSSProperties} data-section-tone={sectionType}>{blocks[type]}</div>; })}</main>;
}
