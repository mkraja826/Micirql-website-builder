...
export function AiEditorAssistant({ site, pageId, sectionId, viewport = "desktop", onApply }: { site: Site; pageId: string; sectionId?: string; viewport?: EditorViewport; onApply(operation: AiEditorOperation): void; }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/design-preferences?${query}`, { headers: { Authorization: `Bearer ${session.access_token}` }, cache: "no-store" })
...
function humanOperation(type: AiEditorOperation["type"]) {
  const labels: Record<string, string> = { "section.variant": "Layout change", "section.copy": "Copy improvement", "section.add": "New section", "section.visibility": "Visibility change", "section.remove": "Remove section", "section.move": "Reorder section", "media.open": "Media change", "functions.open": "Function setup", "seo.patch": "SEO improvement", "page.add": "New page" };
