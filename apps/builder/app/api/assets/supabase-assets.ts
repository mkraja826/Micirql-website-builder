...
export async function uploadAssetObject(workspaceId:string,id:string,dataUrl:string){
 const match=dataUrl.match(/^data:(image\/[a-zA-Z0-9.+-]+);base64,(.+)$/);if(!match)throw statusError(415,"Only base64 image uploads are supported.");const mime=match[1]!,bytes=Uint8Array.from(atob(match[2]!),c=>c.charCodeAt(0));const ext=mime.split("/")[1]?.replace("jpeg","jpg")||"bin";const key=`${workspaceId}/${id}/original.${ext}`;const{url,bucket}=assetConfig();const response=await fetch(`${url}/storage/v1/object/${bucket}/${key}`,{method:"POST",headers:{...serverHeaders(),"content-type":mime,"x-upsert":"false"},body:bytes});if(!response.ok)throw statusError(response.status,await response.text());return{key,url:`${url}/storage/v1/object/public/${bucket}/${key}`};
}
