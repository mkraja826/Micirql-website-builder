...
export async function POST(request:Request){
 try{
  ...
  const auth=`Basic ${btoa(`${keyId}:${keySecret}`)}`;const paymentResponse=await fetch(`https://api.razorpay.com/v1/payments/${encodeURIComponent(paymentId)}`,{headers:{authorization:auth},cache:"no-store"});const payment=await paymentResponse.json() as{id?:string;order_id?:string;status?:string;amount?:number;currency?:string;error_description?:string};if(!paymentResponse.ok)throw statusError(502,payment.error_description??"Could not verify Razorpay payment status.");
  if(payment.order_id!==stored.provider_order_id||payment.status!=="captured"||payment.amount!==stored.amount_inr*100||payment.currency!==stored.currency)throw statusError(409,"PAYMENT_NOT_CAPTURED_OR_MISMATCHED");
