...
export async function PUT(request:NextRequest){let body:unknown;try{body=await request.json()}catch{return NextResponse.json({error:"Invalid JSON"},{status:400})}if(!body||typeof body!=="object")return NextResponse.json({error:"Invalid request"},{status:400});const input=body as Record<string,unknown>;const expectedRevision=Number(input.expectedRevision);const updatedBy=typeof input.updatedBy==="string"&&input.updatedBy.trim()?input.updatedBy.trim():"workspace-user";if(!Number.isInteger(expectedRevision)||expectedRevision<0)return NextResponse.json({error:"expectedRevision must be a non-negative integer"},{status:400});let snapshot:Site;try{snapshot=repairContentDepth(siteSchema.parse(input.snapshot))}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Invalid Site Schema"},{status:422})}try{if(usesSupabaseDraftStore()){const current=await getSupabaseDraft(request,snapshot.workspaceId,snapshot.siteId);snapshot=propagateEditedGlobalShell(snapshot,current?.snapshot);const effectiveRevision=updatedBy==="first-build-review"&&current?current.revision:expectedRevision;const draft=await saveSupabaseDraft(request,{snapshot,expectedRevision:effectiveRevision,updatedBy});return NextResponse.json({draft:withContentDepth(draft)})}const draftKey=key(snapshot.workspaceId,snapshot.siteId);const current=drafts.get(draftKey);const currentRevision=current?.revision??0;const effectiveRevision=updatedBy==="first-build-review"?currentRevision:expectedRevision;if(currentRevision!==effectiveRevision)return NextResponse.json({error:"REVISION_CONFLICT",currentRevision,draft:current},{status:409});snapshot=propagateEditedGlobalShell(snapshot,current?.snapshot);const next:DraftRecord={workspaceId:snapshot.workspaceId,siteId:snapshot.siteId,revision:currentRevision+1,snapshot,updatedAt:new Date().toISOString(),updatedBy};drafts.set(draftKey,next);return NextResponse.json({draft:withContentDepth(next)})}catch(error){const message=error instanceof Error?error.message:"Draft save failed";const status=message==="REVISION_CONFLICT"?409:((error as Error&{status?:number}).status??500);let current:DraftRecord|undefined;if(status===409&&usesSupabaseDraftStore()){try{current=await getSupabaseDraft(request,snapshot.workspaceId,snapshot.siteId)}catch{}}return NextResponse.json({error:message,draft:current?withContentDepth(current):undefined},{status})}}
