...
export async function loadMediaPools(workspaceId:string){
 ...
 return{
  ...
  libraryAssets:assets.filter(a=>!a.workspaceId&&a.source!=="external").map(toMediaAsset),
  licensedAssets:assets.filter(a=>!a.workspaceId&&a.source==="external").map(toMediaAsset)
 };
}
function toMediaAsset(asset:AssetRecord):MediaAsset{
 return{id:asset.id,url:asset.originalUrl,source:asset.source==="user-upload"?"customer":asset.source==="external"?"licensed":"library",tags:[...asset.tags,...asset.sectionFamilies,...asset.domains,...asset.subtypes],alt:asset.alt,aspect:aspect(asset.aspectRatio),verified:asset.source==="user-upload"||asset.license!=="unknown"};
}
