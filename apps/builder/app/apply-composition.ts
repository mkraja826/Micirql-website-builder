...
function mergeThemeKeepingBrand(site:Site,composition:WebsiteComposition,quality?:GenerationQualityProfile){
  ...
  if(selection){
    ...
    theme.brand.colors={primary:p.primary,secondary:p.secondary,accent:p.accent,background:p.background,surface:p.surface,textPrimary:p.text,textSecondary:p.mutedText,border:p.border,success:old.success,warning:old.warning,error:old.error};
...
function sectionQualityScore(section:Site["pages"][number]["sections"][number]){const props=section.props??{},values=sectionTextValues(section);let score=values.join(" ").length;for(const value of values)if(SCAFFOLD_COPY.test(value))score-=250;if(Array.isArray(props.items)&&props.items.length===0)score-=80;return score;}
function isScaffoldSection(section:Site["pages"][number]["sections"][number]){const family=familyFromId(section.component.componentId);if(family==="navbar"||family==="footer"||family==="contact")return false;const props=section.props??{};if(family&&ITEM_CONTENT_FAMILIES.has(family)&&(!Array.isArray(props.items)||props.items.length===0))return true;return sectionTextValues(section).some(value=>SCAFFOLD_COPY.test(value));}
function sectionTextValues(section:Site["pages"][number]["sections"][number]){const props=section.props??{},values:string[]=[];for(const key of ["title","heading","description","body","eyebrow"]){const value=props[key];if(typeof value==="string"&&value.trim())values.push(value.trim());}if(Array.isArray(props.items))for(const raw of props.items){if(!raw||typeof raw!=="object")continue;const item=raw as Record<string,unknown>;for(const key of ["title","description"]){const value=item[key];if(typeof value==="string"&&value.trim())values.push(value.trim());}}return values;}
