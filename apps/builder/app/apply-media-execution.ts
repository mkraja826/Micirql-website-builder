...
export function applyMediaExecution(site:Site,execution:MediaExecutionPlan):Site{
 ...
 for(const page of next.pages){for(const section of page.sections){const family=familyFromId(section.component.componentId);if(!family)continue;const request=queues.get(family)?.shift();if(!request)continue;apply(section.props,request,family);}}
...
function apply(props:Record<string,unknown>,request:MediaRequest,family:SectionFamily){
 ...
 // All universal sections already understand section-level `image`.
 ...
 // Gallery/team/features render item images directly; seed the first truthful slot without overwriting supplied media.
...
function group(requests:MediaRequest[]){const map=new Map<SectionFamily,MediaRequest[]>();for(const request of requests){const list=map.get(request.family)??[];list.push(request);map.set(request.family,list);}return map;}
