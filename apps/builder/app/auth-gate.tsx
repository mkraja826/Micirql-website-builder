...
export function AuthGate({ children }: { children(session: SupabaseSession): React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => { window.fetch = nativeFetch; setAuthFetchReady(false); };
  ...
  if(session&&!authFetchReady)return <>{styles}<main className="auth-shell"><div className="auth-card">Preparing secure session…</div></main></>;
