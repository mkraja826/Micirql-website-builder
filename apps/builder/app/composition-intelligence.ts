...
export function composeWebsite(profile:OnboardingProfile):WebsiteComposition{
 ...
 return{preset,intent,sections,reasoning:[`Selected ${preset.name} from the business description`,`Optimized the narrative for ${intent}`,`Composed ${sections.length} certified section families instead of using a fixed template`,...top.reasons]};
}
