...
type Health = { trustedMeteringConfigured:boolean; meteringMode:"trusted-service-role"|"authenticated-fallback"; aiMeteringCutoverReady?:boolean };
...
function Diagnostics({session}:{session:SupabaseSession}){
 ...
 useEffect(()=>{let cancelled=false;(async()=>{try{let project:{workspaceId?:string;siteId?:string}|null=null;try{project=JSON.parse(window.localStorage.getItem("micirql_active_project")||"null")}catch{}if(!project?.workspaceId)throw new Error("Open a website first so MiCirql knows which workspace diagnostics to show.");const q=new URLSearchParams({workspaceId:project.workspaceId,...(project.siteId?{siteId:project.siteId}:{})});const headers={Authorization:`Bearer ${session.access_token}`};const [r,h]=await Promise.all([fetch(`/api/admin/build-diagnostics?${q}`,{headers,cache:"no-store"}),fetch("/api/admin/build-diagnostics/health",{headers,cache:"no-store"})]);const p=await r.json();const hp=await h.json();if(!r.ok)throw new Error(p.error??"Could not load diagnostics.");if(!cancelled){setSummary(p.summary);setBuilds(p.builds??[]);if(h.ok)setHealth(hp)}}catch(e){if(!cancelled)setError(e instanceof Error?e.message:"Could not load diagnostics.")}finally{if(!cancelled)setLoading(false)}})();return()=>{cancelled=true}},[session.access_token]);
 return <main style={shell}>
  ...
  {loading?<section style={panel}>Loading build telemetry…</section>:error?<section style={{...panel,borderColor:"#7f1d1d"}}>{error}</section>:<>
   <section style={{...panel,marginBottom:18,borderColor:health?.trustedMeteringConfigured?"#14532d":"#854d0e"}}><strong>AI metering security</strong><p style={{...muted,margin:"8px 0 0"}}>{health?.trustedMeteringConfigured?"Trusted service-role metering is configured. The database cutover can safely revoke direct browser metering access.":"MiCirql is still using authenticated fallback metering. Add SUPABASE_SERVICE_ROLE_KEY to the production builder environment before applying the final RPC revoke migration."}</p><div style={{marginTop:10}}><span style={pill(health?.trustedMeteringConfigured?"success":"recovered")}>{health?.meteringMode??"unknown"}</span></div></section>
   <section style={stats}>{[
