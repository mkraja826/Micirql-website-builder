...
export function GuidedOnboarding({ session, workspaceId, siteId, building, error, onBack, onSubmit }: { session: SupabaseSession; workspaceId: string; siteId: string; building: boolean; error?: string; onBack?: () => void; onSubmit(value: GuidedOnboardingValue): Promise<void> | void; }) {
  ...
  async function createWebsite() {
    const context = value.context.trim(); if (context.length < 20) { setLocalError("Tell MiCirql a little more about your business and the website you want."); return; }
    ...
    try {
      ...
      const next: GuidedOnboardingValue = { ...value, businessName: asText(profile.businessName) || "My Business", industry: asText(profile.industry) || "other", subindustry: asText(profile.subindustry), location: asText(profile.location), services: asList(profile.services).join(", "), goals: asList(profile.goals), styleTags: asList(profile.styleTags), requiredCapabilities: asList(profile.requiredCapabilities), languages: asList(profile.languages).join(", ") || "en", notes: asText(profile.notes) || context };
      setValue(next); await onSubmit(next);
    } catch (caught) { setLocalError(caught instanceof Error ? caught.message : "MiCirql could not start the website build."); } finally { setInterpreting(false); }
  }

  const busy = building || logoBusy || assetBusy || interpreting;
  return <main className={styles.shell}><section className={styles.card}>
    <div className={styles.topbar}><button className={styles.back} type="button" onClick={onBack}>← Projects</button><span className={styles.progressMeta}>AI website brief</span></div>
    <div className={styles.content}>
      <header><div className={styles.eyebrow}>MiCirql website intelligence</div><h1 className={styles.heading}>Describe the website you want.</h1><p className={styles.intro}>Write naturally. Tell MiCirql about your business, services, location, audience, style, goals and any features you need. You do not need to fill out a form or choose a template.</p></header>
      <div className={styles.step}>
        <label className={styles.field}>Your website brief<textarea className={styles.control} style={{ minHeight: 250, resize: "vertical", lineHeight: 1.65 }} value={value.context} onChange={(event) => setValue((current) => ({ ...current, context: event.target.value }))} placeholder={'Example: “I run a premium dental implant clinic in Hyderabad called Pearl Dental. We focus on implants, cosmetic dentistry and full-mouth rehabilitation. I want a modern luxury website that builds trust, introduces our doctors, shows before/after work and Google reviews, and encourages patients to book appointments on WhatsApp.”'} autoFocus /></label>
      ...
      <div className={styles.actions}><span className={styles.hint}>{assetBusy ? "Classifying your business media…" : interpreting ? "Understanding your brief…" : building ? "Generating content, imagery and design directions…" : "You can refine everything after generation."}</span><button type="button" className={styles.primary} disabled={busy || value.context.trim().length < 20} onClick={() => void createWebsite()}>{building ? "Building your website…" : interpreting ? "Understanding brief…" : "Create my website"}</button></div>
    </div>
  </section></main>;
}
