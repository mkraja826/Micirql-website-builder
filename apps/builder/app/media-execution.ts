...
export function executeMediaPlan(input:MediaExecutionInput):MediaExecutionPlan{
 ...
 return{requests,generationCount,rules:[...input.plan.rules,"Customer assets always outrank reusable or generated media","Team and leadership portraits are identity-bearing media and must be customer supplied","Certified industry tags strongly influence reusable-media ranking","Blueprint aspect ratio and preferred tags must survive into stock-image retrieval","Dental blueprint support imagery may be generated only when the subject is generic, non-identifying and non-claim","Synthetic before-and-after outcomes, real-team stand-ins, fabricated clinic interiors, fabricated equipment ownership and credentials are never allowed","Do not reuse a primary asset across sections","Generation is a fallback, not a default"]};
}
...
export function canGenerate(d:SectionVisualDecision){
 ...
 if(requiresCustomerIdentityMedia(d))return false;
