...
export function OnboardingGate({ session }: { session: SupabaseSession }) {
  ...
  return <main style={shellStyle}><form onSubmit={submit} style={{ ...cardStyle, maxWidth: 920 }}>
    ...
    <div style={gridStyle}><Field label="Languages"><input value={form.languages} placeholder="en, hi, te" onChange={e => setForm({ ...form, languages: e.target.value })} /></Field><Field label="Anything else"><input value={form.notes} placeholder="International patients, 24/7 enquiries…" onChange={e => setForm({ ...form, notes: e.target.value })} /></Field></div>
