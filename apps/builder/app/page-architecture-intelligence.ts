...
export function planPageArchitecture(input: PageArchitectureInput): PageArchitecturePlan {
  ...
  const pages: ArchitecturePage[] = [
    { id: "home", path: "/", name: "Home", purpose: "Primary conversion and trust page" },
    { id: "about", path: "/about", name: "About", purpose: "Business story, credibility and differentiation" },
  ];

  if (services.length) {
    pages.push({ id: "services", path: "/services", name: healthcare ? "Treatments" : "Services", purpose: "Overview of core offerings" });
    ...
    if (wantsServicePages) {
      for (const service of services.slice(0, healthcare ? 6 : 5)) {
        ...
        if (!slug) continue;
        pages.push({ id: `service-${slug}`, path: `/services/${slug}`, name: titleCase(service), primaryKeyword: service, purpose: `Dedicated ${service} conversion and search page` });
      }
    }
  }

  if (healthcare || /doctor|team|staff|founder|leadership/.test(notes)) {
    pages.push({ id: "team", path: healthcare ? "/doctors" : "/team", name: healthcare ? "Doctors" : "Team", purpose: "Team credibility and human trust" });
  }
  if (capabilities.has("gallery") || /gallery|portfolio|before.?after|case studies|our work/.test(notes)) {
    pages.push({ id: "gallery", path: healthcare ? "/results" : "/gallery", name: healthcare ? "Results" : "Gallery", purpose: "Visual proof and outcomes" });
  }
  if (capabilities.has("blog") || goals.has("rank in search") || /blog|articles|resources|insights/.test(notes)) {
    pages.push({ id: "blog", path: "/blog", name: healthcare ? "Patient Resources" : "Insights", purpose: "Educational and organic search content hub" });
  }
  if (/faq|frequently asked|questions/.test(notes) || healthcare) {
    pages.push({ id: "faq", path: "/faq", name: "FAQs", purpose: "Answer objections and common questions" });
  }
  pages.push({ id: "contact", path: "/contact", name: goals.has("book appointments") ? "Book Appointment" : "Contact", purpose: "Final conversion and contact page" });
...
export function applyPageArchitecture(site: Site, plan: PageArchitecturePlan): Site {
  ...
  if (!home) return site;
  const shellStart = home.sections.filter((section) => section.component.componentId.toLowerCase().includes("nav"));
  const shellEnd = home.sections.filter((section) => section.component.componentId.toLowerCase().includes("foot"));
  const body = home.sections.filter((section) => !shellStart.includes(section) && !shellEnd.includes(section));

  next.pages = plan.pages.map((pagePlan, index) => {
    ...
    if (!existing && index > 0) base.sections = [...structuredClone(shellStart), ...structuredClone(body), ...structuredClone(shellEnd)];
    return base;
  });
