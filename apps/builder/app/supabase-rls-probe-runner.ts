...
export type SupabaseProbeQueryExecutor = (projectRef: string, sql: string, readOnly: boolean) => Promise<unknown>;

const IDENTIFIER = /^[a-z_][a-z0-9_]*$/;

export function createSupabaseRlsProbeRunner(query: SupabaseProbeQueryExecutor) {
  return async function runSupabaseRlsProbes(projectRef: string, contract: BackendImplementationContract): Promise<SupabaseCertificationProbeResult> {
    const errors: string[] = [];
    let positiveRlsPassed: boolean | undefined;
    let negativeRlsPassed: boolean | undefined;

    const protectedTable = contract.tables.find((table) => table.rlsEnabled && table.columns.some((column) => column.name === "owner_user_id" || column.name === "tenant_id"));
    ...
    const hasOwnerScopedStorage = contract.storageBuckets.some((bucket) => bucket.ownerScoped);
    return {
      ...(positiveRlsPassed !== undefined ? { positiveRlsPassed } : {}),
      ...(negativeRlsPassed !== undefined ? { negativeRlsPassed } : {}),
      ...(!hasOwnerScopedStorage ? { storageOwnershipPassed: true } : {}),
      errors,
    };
  };
}

async function probeProtectedTable(query: SupabaseProbeQueryExecutor, projectRef: string, table: BackendTable, contract: BackendImplementationContract) {
  ...
  try {
    ...
    if (tenantColumn && contract.tables.some((candidate) => candidate.name === "tenants")) await safeCleanup(query, projectRef, "tenants", [tenantA, tenantB]);
  }
}
...
async function safeCleanup(query: SupabaseProbeQueryExecutor, projectRef: string, table: string, ids: string[]) { try { assertIdentifier(table); await query(projectRef, `delete from public.${q(table)} where "id" in (${ids.map(sqlString).join(", ")});`, false); } catch { /* disposable preview cleanup */ } }
function probeValue(type: BackendTable["columns"][number]["type"], name: string) { if (type === "uuid") return sqlString(crypto.randomUUID()); if (type === "integer" || type === "numeric") return "1"; if (type === "boolean") return "false"; if (type === "date") return "current_date"; if (type === "timestamptz") return "now()"; if (type === "jsonb") return "'{}'::jsonb"; return sqlString(`MiCirql probe ${name}`); }
function scalarCount(value: unknown) { const rows = normalizeRows(value); const count = rows[0]?.count; if (typeof count === "number") return count; if (typeof count === "string" && /^\d+$/.test(count)) return Number(count); return -1; }
function normalizeRows(value: unknown): Array<Record<string, unknown>> { if (Array.isArray(value)) return value.filter(isRecord); if (isRecord(value)) { for (const key of ["result", "data", "rows"]) { const candidate = value[key]; if (Array.isArray(candidate)) return candidate.filter(isRecord); } } return []; }
