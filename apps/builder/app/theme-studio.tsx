...
export function ThemeStudio({ theme, onChange }: { theme: ThemeConfig; onChange(next: ThemeConfig): void }) {
  ...
  const setFamily = (family: ThemeConfig["family"]) => onChange({ ...theme, family, modifiers: theme.modifiers.filter((value)=>value!=="dark"&&value!=="light").concat(theme.brand.colors.background.toLowerCase()==="#080b12"?["dark"]:["light"]).slice(0,3) as ThemeConfig["modifiers"] });

  return <div className="theme-studio">
    <section><span className="theme-studio-label">Theme</span><div className="theme-chip-grid">{FAMILIES.map(f=><button type="button" key={f} className={theme.family===f?"is-active":""} onClick={()=>setFamily(f)}>{f}</button>)}</div></section>
    <section><span className="theme-studio-label">Palette</span><div className="palette-grid">{PALETTES.map(p=><button type="button" key={p.id} onClick={()=>patchBrand({colors:p.colors})}><span className="palette-swatch" style={{background:`linear-gradient(135deg, ${p.colors.primary} 0 50%, ${p.colors.background} 50%)`}}/><span>{p.name}</span></button>)}</div></section>
    <section><span className="theme-studio-label">Typography</span><div className="segmented-grid">{TYPE.map(t=><button type="button" key={t.id} onClick={()=>patchBrand({typography:t.value})}><strong style={{fontFamily:t.value.display}}>{t.name}</strong><small>{t.value.display}</small></button>)}</div></section>
    <section className="theme-control-row"><label>Density<select value={theme.brand.density} onChange={e=>patchBrand({density:e.target.value as ThemeConfig["brand"]["density"]})}>{DENSITIES.map(v=><option key={v}>{v}</option>)}</select></label><label>Shape<select value={theme.brand.shape} onChange={e=>patchBrand({shape:e.target.value as ThemeConfig["brand"]["shape"]})}>{SHAPES.map(v=><option key={v}>{v}</option>)}</select></label><label>Motion<select value={theme.brand.motion} onChange={e=>patchBrand({motion:e.target.value as ThemeConfig["brand"]["motion"]})}>{MOTIONS.map(v=><option key={v}>{v}</option>)}</select></label></section>
    <section><span className="theme-studio-label">Fine tune</span><div className="theme-color-pickers"><label>Primary<input type="color" value={theme.brand.colors.primary} onChange={e=>patchBrand({colors:{...theme.brand.colors,primary:e.target.value}})}/></label><label>Accent<input type="color" value={theme.brand.colors.accent} onChange={e=>patchBrand({colors:{...theme.brand.colors,accent:e.target.value}})}/></label><label>Background<input type="color" value={theme.brand.colors.background} onChange={e=>patchBrand({colors:{...theme.brand.colors,background:e.target.value}})}/></label></div></section>
  </div>;
}
