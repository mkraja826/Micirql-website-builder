...
export function ThemeStudio({ theme, onChange }: { theme: ThemeConfig; onChange(next: ThemeConfig): void }) {
  ...
  return <div className={styles.studio} role="region" aria-label="Site-wide design studio" aria-describedby="theme-studio-scope">
    ...
    <section className={styles.section}><span className={styles.label} id="theme-presets-label">Theme</span><div className={styles.chips} role="group" aria-labelledby="theme-presets-label">{FAMILIES.map(f=><button type="button" key={f} className={theme.family===f?styles.active:undefined} aria-pressed={theme.family===f} onClick={()=>setFamily(f)}>{humanLabel(f)}</button>)}</div></section>
    <section className={styles.section}><span className={styles.label} id="palette-presets-label">Palette</span><div className={styles.palette} role="group" aria-labelledby="palette-presets-label">{PALETTES.map(p=>{const active=sameRecord(theme.brand.colors,p.colors);return <button type="button" key={p.id} className={active?styles.active:undefined} aria-pressed={active} onClick={()=>patchBrand({colors:p.colors})}><span className={styles.swatch} style={{background:`linear-gradient(135deg, ${p.colors.primary} 0 50%, ${p.colors.background} 50%)`}}/><span>{p.name}</span></button>})}</div></section>
    <section className={styles.section}><span className={styles.label} id="typography-presets-label">Typography</span><div className={styles.type} role="group" aria-labelledby="typography-presets-label">{TYPE.map(t=>{const active=sameRecord(theme.brand.typography,t.value);return <button type="button" key={t.id} className={active?styles.active:undefined} aria-pressed={active} onClick={()=>patchBrand({typography:t.value})}><strong style={{fontFamily:t.value.display}}>{t.name}</strong><small>{t.value.display}</small></button>})}</div></section>
    <section className={styles.controls} role="group" aria-label="Layout and motion controls">
      <label>Density<small id="theme-density-help">Controls overall spacing and content breathing room.</small><select aria-describedby="theme-density-help" value={theme.brand.density} onChange={e=>patchBrand({density:e.target.value as ThemeConfig["brand"]["density"]})}>{DENSITIES.map(v=><option key={v} value={v}>{humanLabel(v)}</option>)}</select></label>
      <label>Shape<small id="theme-shape-help">Controls how sharp or rounded the site feels.</small><select aria-describedby="theme-shape-help" value={theme.brand.shape} onChange={e=>patchBrand({shape:e.target.value as ThemeConfig["brand"]["shape"]})}>{SHAPES.map(v=><option key={v} value={v}>{humanLabel(v)}</option>)}</select></label>
      <label>Motion<small id="theme-motion-help">Controls the amount of site-wide interface animation.</small><select aria-describedby="theme-motion-help" value={theme.brand.motion} onChange={e=>patchBrand({motion:e.target.value as ThemeConfig["brand"]["motion"]})}>{MOTIONS.map(v=><option key={v} value={v}>{humanLabel(v)}</option>)}</select></label>
    </section>
    <section className={styles.section}><span className={styles.label} id="fine-tune-colors-label">Fine tune</span><div className={styles.pickers} role="group" aria-labelledby="fine-tune-colors-label">
      <label>Primary<span id="theme-primary-value">{theme.brand.colors.primary}</span><input type="color" aria-label="Primary color" aria-describedby="theme-primary-value" value={theme.brand.colors.primary} onChange={e=>patchBrand({colors:{...theme.brand.colors,primary:e.target.value}})}/></label>
      <label>Accent<span id="theme-accent-value">{theme.brand.colors.accent}</span><input type="color" aria-label="Accent color" aria-describedby="theme-accent-value" value={theme.brand.colors.accent} onChange={e=>patchBrand({colors:{...theme.brand.colors,accent:e.target.value}})}/></label>
      <label>Background<span id="theme-background-value">{theme.brand.colors.background}</span><input type="color" aria-label="Background color" aria-describedby="theme-background-value" value={theme.brand.colors.background} onChange={e=>patchBrand({colors:{...theme.brand.colors,background:e.target.value}})}/></label>
    </div></section>
  </div>;
}
