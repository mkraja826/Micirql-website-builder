...
export default function WorkspaceClient() {
  ...
  const currentAssetId = activeSection && typeof activeSection.props.image === "object" && activeSection.props.image !== null && !Array.isArray(activeSection.props.image)
    ? String((activeSection.props.image as Record<string, unknown>).assetId ?? "") || undefined
    : undefined;
  ...
  return <main className="workspace-shell">
    ...
    <aside className="workspace-inspector"><div className="inspector-heading"><span>{mode}</span><strong>{activeSection ? activeSection.id : activePage.name}</strong></div>
      ...
      : mode === "images" && activeSection && activeSectionFamily ? <AssetPicker workspaceId={state.site.workspaceId} siteId={state.site.siteId} pagePath={activePage.path} sectionId={activeSection.id} domain={state.site.domain} theme={state.site.theme.family} family={activeSectionFamily} currentAssetId={currentAssetId} onSelect={(asset, overrides) => commit({ type: "asset.set", pageId: activePage.id, sectionId: activeSection.id, propPath: "image", asset: { assetId: asset.id, alt: overrides?.alt ?? asset.alt, focalPoint: overrides?.focalPoint ?? asset.focalPoint } })} />
      : mode === "pages" ? <PagesManager site={state.site} activePageId={activePage.id} onSelect={selectPage} onAdd={(page) => { commit({ type: "page.add", page }); selectPage(page.id); }} onRemove={(pageId) => { commit({ type: "page.remove", pageId }); if (pageId === activePage.id) selectPage(state.site.pages[0]!.id); }} onDuplicate={(page) => { commit({ type: "page.add", page }); selectPage(page.id); }} onPathChange={(pageId, path) => commit({ type: "page.path.set", pageId, path })} onReorder={(pageId, toIndex) => commit({ type: "page.reorder", pageId, toIndex })} />
      ...
      : mode === "functions" ? <div className="inspector-empty"><p>Select a section to configure its functionality.</p><span>MiCirql only exposes approved registered actions here.</span></div>
    ...
    <nav className="mobile-editor-bar" aria-label="Editing tools">{(["content", "images", "design", "pages", "seo", "functions"] as Mode[]).map((item) => <button key={item} className={mode === item ? "is-active" : ""} onClick={() => setMode(item)}>{item}</button>)}</nav>
  </main>;
}
