...
export default function WorkspaceClient({session,workspaceId,siteId}:{session:SupabaseSession;workspaceId:string;siteId:string}){
 ...
 useEffect(()=>{let cancelled=false;loaded.current=false;setSaveState("loading");(async()=>{try{const q=new URLSearchParams({workspaceId,siteId});const r=await fetch(`/api/drafts?${q}`,{headers:{Authorization:`Bearer ${session.access_token}`},cache:"no-store"});if(cancelled)return;if(r.status===404){setSaveState("error");loaded.current=true;return}if(!r.ok)throw new Error(`Draft load failed (${r.status}).`);const p=await r.json() as {draft:DraftApiRecord};const site=siteSchema.parse(p.draft.snapshot);const editor=createEditorState(site);editor.revision=p.draft.revision;setHistory(createEditorHistory(editor));setPersistedRevision(p.draft.revision);setSaveState("saved");loaded.current=true}catch(e){if(!cancelled){console.error(e);setSaveState("error");loaded.current=true}}})();return()=>{cancelled=true}},[session.access_token,workspaceId,siteId]);
 ...
 async function persist(snapshot:Site,sentEditorRevision:number):Promise<boolean>{if(saveState==="conflict"||saveState==="saving")return false;setSaveState("saving");try{const r=await fetch("/api/drafts",{method:"PUT",headers:{Authorization:`Bearer ${session.access_token}`,"content-type":"application/json"},body:JSON.stringify({snapshot,expectedRevision:persistedRevision,updatedBy:"workspace-user"})});const p=await r.json() as {draft?:DraftApiRecord;error?:string};if(r.status===409){setConflictDraft(p.draft);setSaveState("conflict");return false}if(!r.ok||!p.draft)throw new Error(p.error??`Draft save failed (${r.status}).`);setPersistedRevision(p.draft.revision);setHistory(c=>({...c,present:c.present.revision===sentEditorRevision?markEditorSaved(c.present):c.present}));setSaveState(c=>c==="conflict"?c:"saved");return true}catch(e){console.error(e);setSaveState("error");return false}}
