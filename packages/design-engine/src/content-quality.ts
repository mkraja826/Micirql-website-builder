...
export function normalizeWebsiteContent(site: Site): Site {
  ...
  for (const page of next.pages) {
    ...
    for (const section of page.sections) {
      ...
      if (typeof props.heading === "string") props.heading = clipWords(cleanText(props.heading), titleLimit);
      ...
      if (typeof props.body === "string") props.body = clipWords(cleanText(props.body), LIMITS.descriptionWords);
      if (typeof props.summary === "string") props.summary = clipWords(cleanText(props.summary), LIMITS.descriptionWords);
      if (typeof props.intro === "string") props.intro = clipWords(cleanText(props.intro), LIMITS.descriptionWords);
      if (typeof props.copy === "string") props.copy = clipWords(cleanText(props.copy), LIMITS.descriptionWords);
      if (typeof props.text === "string") props.text = clipWords(cleanText(props.text), LIMITS.descriptionWords);
...
export function evaluateWebsiteContent(site: Site): ContentQualityResult {
  ...
  for(const page of site.pages){
    ...
    for(const section of page.sections){
      ...
      const props=section.props??{};
      const family=familyFromComponentId(section.component.componentId);
      const title=text(props.title)||text(props.heading);
      const description=text(props.description)||text(props.body)||text(props.summary)||text(props.intro)||text(props.copy)||text(props.text);
      const titlePath=text(props.title)?"title":text(props.heading)?"heading":"title";
      const descriptionPath=text(props.description)?"description":text(props.body)?"body":text(props.summary)?"summary":text(props.intro)?"intro":text(props.copy)?"copy":text(props.text)?"text":"description";
      ...
      if(!title) issues.push(issue("MISSING_SECTION_TITLE","error","Visible sections need a clear title.",page.id,section.id,titlePath));
      if(title){const max=family==="hero"?LIMITS.heroTitleWords:LIMITS.sectionTitleWords; if(wordCount(title)>max) issues.push(issue("TITLE_TOO_LONG","warning",`${family??"Section"} title is too long for its layout.`,page.id,section.id,titlePath)); detectDuplicate(title,page.id,section.id,seenPhrases,issues); detectPlaceholder(title,page.id,section.id,titlePath,issues); detectGenericAiCopy(title,page.id,section.id,titlePath,issues); const entry={text:title,pageId:page.id,sectionId:section.id,path:titlePath,family}; semanticCopy.push(entry); pageSemantic.push(entry);}
      if(description){if(wordCount(description)>LIMITS.descriptionWords) issues.push(issue("DESCRIPTION_TOO_LONG","warning","Section paragraph is too dense.",page.id,section.id,descriptionPath)); detectPlaceholder(description,page.id,section.id,descriptionPath,issues); detectGenericAiCopy(description,page.id,section.id,descriptionPath,issues); collectOpening(description,page.id,section.id,openingCounts); const entry={text:description,pageId:page.id,sectionId:section.id,path:descriptionPath,family}; semanticCopy.push(entry); pageSemantic.push(entry);}
