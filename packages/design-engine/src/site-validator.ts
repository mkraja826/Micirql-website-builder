...
export function validateWebsite(site: Site, archetypeId: string): WebsiteValidationResult {
  ...
  for (const shellFamily of SITE_INVARIANTS.requiredShell) {
    if (!homeFamilies.includes(shellFamily)) issues.push(error("MISSING_SITE_SHELL", `${shellFamily} is mandatory on the home page.`, home.id));
  }
  ...
  if (archetype) {
    for (const requirement of archetype.sections.required) {
      if (!satisfiesRequirement(homeFamilies, requirement)) {
        issues.push(error("MISSING_ARCHETYPE_SECTION", `${archetype.name} requires a ${requirement} section.`, home.id));
      }
    }
    for (const recommendation of archetype.sections.recommended) {
      if (!satisfiesRequirement(homeFamilies, recommendation)) {
        issues.push(warning("MISSING_RECOMMENDED_SECTION", `${archetype.name} usually benefits from a ${recommendation} section.`, home.id));
      }
    }
  }

  const visibleSections = site.pages.flatMap((page) => page.sections.filter((section) => !section.hidden).map((section) => ({ page, section })));
  const hasPrimaryAction = visibleSections.some(({ section }) => hasAction(section.props?.primaryAction));
  if (!hasPrimaryAction) issues.push(error("MISSING_PRIMARY_CTA", "Website must contain at least one working primary CTA."));
  ...
  if (navbar && !isMiCirqlNavbar(navbar.section.component.componentId)) {
    issues.push(error("MOBILE_NAV_UNVERIFIED", "Navbar must use a MiCirql shell component with guaranteed mobile burger navigation.", navbar.page.id, navbar.section.id));
  }
...
function validateImageSlot(props: Record<string, unknown> | undefined, issues: WebsiteValidationIssue[], pageId: string, sectionId: string) {
  ...
  if (mode !== undefined && (typeof mode !== "string" || !VALID_IMAGE_MODES.has(mode))) {
    issues.push(error("INVALID_IMAGE_SLOT_MODE", "Image slot mode is invalid.", pageId, sectionId));
  }
  for (const key of ["imageRatio", "itemImageRatio"] as const) {
    const ratio = props[key];
    if (ratio !== undefined && (typeof ratio !== "string" || !VALID_IMAGE_RATIOS.has(ratio))) {
      issues.push(error("INVALID_IMAGE_RATIO", `${key} is invalid.`, pageId, sectionId));
    }
  }
  if ((mode === "section" || mode === "both") && !props.imageRatio) {
    issues.push(error("MISSING_SECTION_IMAGE_RATIO", "Section image slot requires an explicit image ratio.", pageId, sectionId));
  }
  if ((mode === "items" || mode === "both") && !props.itemImageRatio) {
    issues.push(error("MISSING_ITEM_IMAGE_RATIO", "Item image slots require an explicit image ratio.", pageId, sectionId));
  }
}
...
function familyFromComponentId(componentId: string): string | undefined {
  ...
  for (const family of Object.keys(FAMILY_ALIASES)) {
    if (value === `${family}.placeholder` || value.startsWith(`${family}.`)) return family;
  }
...
function isMiCirqlNavbar(componentId: string): boolean {
  const value = componentId.toLowerCase();
  return value.startsWith("navbar.") || value.includes("-nav-");
}

function error(code: string, message: string, pageId?: string, sectionId?: string): WebsiteValidationIssue {
  return { code, severity: "error", message, ...(pageId ? { pageId } : {}), ...(sectionId ? { sectionId } : {}) };
}

function warning(code: string, message: string, pageId?: string, sectionId?: string): WebsiteValidationIssue {
  return { code, severity: "warning", message, ...(pageId ? { pageId } : {}), ...(sectionId ? { sectionId } : {}) };
}

function finish(issues: WebsiteValidationIssue[]): WebsiteValidationResult {
  const errors = issues.filter((issue) => issue.severity === "error");
  const warnings = issues.filter((issue) => issue.severity === "warning");
  const score = Math.max(0, 100 - errors.length * 15 - warnings.length * 3);
  return { ready: errors.length === 0, score, errors, warnings };
}
