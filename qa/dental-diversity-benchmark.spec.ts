...
const scenarios: Array<{ id: string; expectedLayout: string; profile: OnboardingProfile }> = [
  { id: "general", expectedLayout: "dental-01-clinical-authority", profile: { industry: "dental clinic", subindustry: "general dentistry", goals: ["book appointments", "build trust"], style_tags: ["clean", "professional"], required_capabilities: ["booking", "contact"], services: ["checkups", "root canal", "crowns"] } },
  { id: "implants", expectedLayout: "dental-02-implant-luxury", profile: { industry: "dental clinic", subindustry: "implant dentistry", goals: ["book implant consultations", "build credibility"], style_tags: ["premium", "elegant"], required_capabilities: ["booking", "before after gallery"], services: ["dental implants", "full arch implants", "implant crowns"] } },
  { id: "cosmetic", expectedLayout: "dental-03-smile-studio", profile: { industry: "dental clinic", subindustry: "cosmetic dentistry", goals: ["showcase smile transformations", "book consultations"], style_tags: ["premium", "visual"], required_capabilities: ["gallery", "booking"], services: ["veneers", "smile design", "teeth whitening"] } },
  { id: "orthodontic", expectedLayout: "dental-09-ortho-journey", profile: { industry: "dental clinic", subindustry: "orthodontics", goals: ["book aligner consultations", "build trust"], style_tags: ["modern", "professional"], required_capabilities: ["booking", "treatment process"], services: ["clear aligners", "braces", "retainers"] } },
  { id: "emergency", expectedLayout: "dental-10-emergency-trust", profile: { industry: "dental clinic", subindustry: "emergency dentistry", goals: ["urgent appointments", "contact clinic"], style_tags: ["clear", "professional"], required_capabilities: ["contact", "booking"], services: ["tooth pain", "broken tooth", "emergency care"] } },
  { id: "family", expectedLayout: "dental-04-family-care", profile: { industry: "dental clinic", subindustry: "family dentistry", goals: ["book family appointments", "build trust"], style_tags: ["welcoming", "clean"], required_capabilities: ["booking", "team"], services: ["preventive dentistry", "fillings", "gum care"] } },
  { id: "endodontic", expectedLayout: "dental-05-digital-dentistry", profile: { industry: "dental clinic", subindustry: "endodontics", goals: ["book specialist consultations", "explain treatment"], style_tags: ["clinical", "professional"], required_capabilities: ["booking", "treatment process"], services: ["root canal treatment", "retreatment", "dental pain assessment"] } },
  { id: "restorative", expectedLayout: "dental-12-wellness-calm", profile: { industry: "dental clinic", subindustry: "restorative dentistry", goals: ["build credibility", "book consultations"], style_tags: ["premium", "calm"], required_capabilities: ["booking", "gallery"], services: ["crowns", "bridges", "full mouth rehabilitation"] } },
  { id: "smile-design", expectedLayout: "dental-03-smile-studio", profile: { industry: "dental clinic", subindustry: "smile design", goals: ["showcase portfolio", "book smile consultations"], style_tags: ["luxury", "visual"], required_capabilities: ["gallery", "booking"], services: ["digital smile design", "veneers", "whitening"] } },
  { id: "implant-education", expectedLayout: "dental-13-implant-results", profile: { industry: "dental clinic", subindustry: "implant dentistry", goals: ["learn about implant treatment", "book consultations"], style_tags: ["premium", "educational"], required_capabilities: ["treatment process", "booking"], services: ["single tooth implants", "implant bridges", "full arch rehabilitation"] } },
];
...
test("dental generation maintains meaningful composition and layout-selection diversity", async () => {
  const results = scenarios.map(({ id, expectedLayout, profile }) => ({ id, expectedLayout, ...fingerprint(profile) }));
  ...
  const summary = {
    ...
    benchmark: "dental-composition-diversity-v2",
  ...
  await writeFile(path.join(outputDirectory, "summary.md"), [
    ...
    "",
    "| Scenario | Expected layout | Selected layout | Score | Intent | Recipe |",
    "| --- | --- | --- | ---: | --- | --- |",
    ...results.map((result) => `| ${result.id} | ${result.expectedLayout} | ${result.layout} | ${result.layoutScore} | ${result.intent} | ${result.recipe} |`),
    "",
  ].join("\n"), "utf8");
