...
test("Top-20 blueprints carry enforceable geometry and mobile safety contracts", async () => {
  ...
  for (const layout of DENTAL_LAYOUT_BLUEPRINTS) {
    ...
    // Portrait-led/editorial spreads, full-bleed openings and typography/whitespace
    // systems are all valid composition systems even without grid/split wording.
    expect(
      desktopRules.some((rule) => /(grid|column|split|asym|media|image|hero|layout|composition|width|span|offset|portrait|spread|mosaic|full[- ]bleed|cinematic|photograph|typographic hierarchy|whitespace|content measure)/.test(rule)),
