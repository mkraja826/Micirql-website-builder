...
for (const fixture of fixtures) {
  for (const candidateId of candidateIds) {
    ...
    const metrics = await page.evaluate((forbidden) => {
      ...
      const primaryCtas = [...document.querySelectorAll('[data-content-primary-cta]')]
        .map((node) => node.getAttribute('data-content-primary-cta') ?? '')
        .filter(Boolean);
      return {
        renderedFixture:main?.getAttribute('data-benchmark-fixture') ?? '',
        candidateId:main?.getAttribute('data-candidate-id') ?? '',
        industry:main?.getAttribute('data-content-industry') ?? '',
        subIndustry:main?.getAttribute('data-content-sub-industry') ?? '',
        pageCount:Number(main?.getAttribute('data-content-page-count') ?? 0),
        sectionCount:Number(main?.getAttribute('data-content-section-count') ?? 0),
        warningCount:Number(main?.getAttribute('data-content-warning-count') ?? 0),
        primaryAction:main?.getAttribute('data-content-primary-action') ?? '',
        sectionTypes:sections.map((node) => node.getAttribute('data-content-section') ?? ''),
        claimCounts:sections.map((node) => Number(node.getAttribute('data-content-claim-count') ?? -1)),
        primaryCtas,
        seoDescription:(document.querySelector('[data-content-seo-description]')?.textContent ?? '').trim(),
        forbiddenMatches:forbidden.filter((term) => bodyText.toLowerCase().includes(term.toLowerCase())),
        faqCount:document.querySelectorAll('[data-content-faq]').length,
      };
    }, forbiddenText);
...
report.summary = {
  ...
  minimumSectionCount:Math.min(...report.candidates.map((candidate) => candidate.sectionCount)),
  maximumWarningCount:Math.max(...report.candidates.map((candidate) => candidate.warningCount)),
  faqPlans:report.candidates.filter((candidate) => candidate.faqCount >= 3).length,
};
