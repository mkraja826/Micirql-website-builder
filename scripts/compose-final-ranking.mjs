...
const rendered = JSON.parse(fs.readFileSync('artifacts/pearl-post-repair-audit/report.json', 'utf8'));
const perceptual = JSON.parse(fs.readFileSync('artifacts/pearl-perceptual-ranking/report.json', 'utf8'));
if (rendered.evidenceState !== 'post-repair') throw new Error('Final ranking requires explicitly post-repair rendered evidence');

const canonicalRanking = rankCandidates(generatePearlDentalCandidates(20), createPearlDentalBrief(), PEARL_DENTAL_KNOWLEDGE);
...
if (renderedById.size !== canonicalRanking.length || perceptualById.size !== canonicalRanking.length) throw new Error('Final ranking evidence must exactly cover the canonical candidate set');
for (const id of renderedById.keys()) if (!canonicalIds.has(id)) throw new Error(`Post-repair rendered evidence contains unknown candidate ${id}`);
...
const candidates = canonicalRanking.map(({ candidate, score, strengths: planStrengths, cautions: planCautions }) => {
  ...
  return {
    ...
    repairInstructionCount: renderedCandidate.repairInstructionCount,
...
const report = {
  ...
  evidenceState: 'post-repair',
  methodology: { weights, note: 'Plan fit comes from the canonical MiCirql ranker. Rendered health comes only from the verified post-repair browser audit. Perceptual quality receives the largest weight. Post-repair hard failures remain disqualifying regardless of score.' },
  candidates,
  summary: {
...
if (candidates.some((candidate) => candidate.hardFailures.length > 0)) { console.error('Final ranking contains candidate(s) with post-repair rendered hard failures.'); process.exit(1); }
if (report.summary.spread < 5) { console.error(`Final ranking is insufficiently discriminating: spread ${report.summary.spread} < 5`); process.exit(1); }
