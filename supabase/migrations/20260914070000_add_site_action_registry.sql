create table if not exists public.site_action_registry (
  ...
  constraint site_action_registry_handler_kind_check
    check (handler_kind in ('lead_request')),
  constraint site_action_registry_status_check
    check (status in ('draft', 'active', 'retired')),
  constraint site_action_registry_action_id_check
    check (action_id ~ '^[a-z0-9][a-z0-9._:-]*$'),
  constraint site_action_registry_action_version_check
    check (action_version ~ '^[a-z0-9][a-z0-9._:-]*$'),
  constraint site_action_registry_capabilities_check
    check (cardinality(capability_keys) > 0)
);

alter table public.site_action_registry enable row level security;

create policy site_action_registry_select_authenticated
on public.site_action_registry
for select
to authenticated
using (true);

grant select on public.site_action_registry to authenticated;
revoke insert, update, delete on public.site_action_registry from anon, authenticated;

create table if not exists public.site_action_bindings (
  ...
  constraint site_action_bindings_action_fk
    foreign key (action_id, action_version)
    references public.site_action_registry(action_id, action_version),
  constraint site_action_bindings_status_check
    check (status in ('disabled', 'active')),
  constraint site_action_bindings_verified_by_check
    check (verified_by = 'system'),
  constraint site_action_bindings_evidence_check
    check (length(trim(verification_evidence_id)) > 0)
);

alter table public.site_action_bindings enable row level security;

create policy site_action_bindings_select_member
on public.site_action_bindings
for select
to authenticated
using (
  exists (
    select 1
    from public.sites s
    where s.id = site_action_bindings.site_id
      and public.is_workspace_member(s.workspace_id)
  )
);

grant select on public.site_action_bindings to authenticated;
revoke insert, update, delete on public.site_action_bindings from anon, authenticated;

insert into public.site_action_registry (
  action_id,
  action_version,
  handler_kind,
  capability_keys,
...
values (
  ...
  jsonb_build_object(
    ...
    'guarantees', jsonb_build_array('submission_recorded'),
    'does_not_guarantee', jsonb_build_array('booking_confirmed', 'reservation_confirmed', 'appointment_confirmed'),
    'requires', jsonb_build_array('name', 'consent', 'email_or_phone')
  )
)
on conflict (action_id, action_version) do update
set handler_kind = excluded.handler_kind,
...
begin
  ...
  select public.consume_site_function_rate_limit(
