...
begin
  ...
  -- A new revision must be exactly the next revision. Replaying an already persisted
  -- revision is allowed only when its immutable payload and certification are identical.
  if not exists (
