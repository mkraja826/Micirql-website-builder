...
begin
  ...
  v_goal_text:=lower(concat_ws(' ',v_primary_goal,coalesce(v_plan.request->>'notes',''),coalesce(v_plan.request->>'industry','')));
  ...
  */
  if v_layout_id is null then
    /*
      Choose a compact narrative arc from the brief instead of appending every
      generic family. The planner may still supply additional sections; this
      only fills the missing minimum for an intentional first composition.
    */
    for v_family in
      select unnest(
        case
          when v_goal_text like '%gallery%'
            or v_goal_text like '%portfolio%'
            or v_goal_text like '%showcase%'
            or v_goal_text like '%project%'
            then array['about','services','gallery','cta']::text[]
          when v_goal_text like '%trust%'
            or v_goal_text like '%testimonial%'
            or v_goal_text like '%credib%'
            then array['about','services','process','testimonials','cta']::text[]
          when v_goal_text like '%lead%'
            or v_goal_text like '%enquir%'
            or v_goal_text like '%appointment%'
            or v_goal_text like '%book%'
            or v_goal_text like '%contact%'
            then array['about','services','process','cta']::text[]
          else array['about','services','process','cta']::text[]
        end
      )
    loop
      if not exists (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'props',(
      case coalesce(s->>'family','')
        ...
        when 'services' then jsonb_build_object('eyebrow',case when v_industry in ('dental','clinic','healthcare') then 'Treatments' else 'Services' end,'title',case when v_industry in ('dental','clinic','healthcare') then 'Treatments' else 'Services' end,'description',case when v_industry in ('dental','clinic','healthcare') then 'Explore the treatments supplied by the clinic.' else 'Explore the services provided by the business.' end,'items',coalesce((select jsonb_agg(jsonb_build_object('title',service_name,'description','Review the information supplied by the business for this service.') order by service_ord) from jsonb_array_elements_text(v_services) with ordinality service_values(service_name,service_ord)),'[]'::jsonb))
        when 'about' then jsonb_build_object('eyebrow','Our approach','title','A clearer experience for every enquiry','description','Explore the information and services supplied by the business, with a clear next step for visitors.','items',jsonb_build_array(jsonb_build_object('title','Grounded content','description','Only information supplied or verified by the business should be published.'),jsonb_build_object('title','Clear next steps','description','Make it easy for visitors to understand what to do next.')))
        when 'features' then jsonb_build_object('eyebrow','Why this page works','title','Designed around clarity and trust','description','A considered structure for the information, services and actions supplied in the brief.','items',jsonb_build_array(jsonb_build_object('title','Service clarity','description','Present the supplied services in a simple, scannable format.'),jsonb_build_object('title','Confidence to act','description','Keep the primary enquiry path visible and easy to follow.'),jsonb_build_object('title','Factual by default','description','Avoid unsupported claims, ratings, outcomes and credentials.')))
        when 'gallery' then jsonb_build_object('eyebrow','Gallery','title','A visual story for the business','description','Verified imagery supplied by the business can be presented here.','items',jsonb_build_array(jsonb_build_object('title','Add verified imagery','description','Replace these visual slots with approved business photography.')))
        ...
        when 'testimonials' then jsonb_build_object('eyebrow','Verified feedback','title','What people can expect','description','Approved feedback supplied by the business can be presented here.','items',jsonb_build_array(jsonb_build_object('title','Verified feedback slot','description','Publish only feedback approved by the business.')))
        when 'process' then jsonb_build_object('eyebrow','Next steps','title','A simple path forward','description','A clear sequence helps visitors understand what happens next.','items',jsonb_build_array(jsonb_build_object('title','Explore','description','Review the information and services supplied by the business.'),jsonb_build_object('title','Ask','description','Use the enquiry path to share what you need.'),jsonb_build_object('title','Continue','description','The business can confirm the appropriate next step.')))
